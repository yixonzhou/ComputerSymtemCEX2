 ...
 * @return 创建后的节点指针
 */
FileSystemNode *filesystem_node_create(FileSystemNodeType type, const char* name, void* data)
{
    // todo 检查参数合法性
    ...
    {
        /* 创建一个空的目录链表 */
        node->data = clist_create(alloc_memory, free_memory);
    } else
    {
        // todo 未知类型
        free_memory(node);
...
void filesystem_init()
{
    ...
    if (f->magic_number != MAGIC_NUMBER_INITED)
    {
        ...
        /* 创建释放后的内存列表 */
        f->unused_nodes = clist_create(alloc_memory, free_memory);

        /* 创建根目录 */
        f->root = filesystem_node_create(Directory, "/", nullptr);
        f->cur_dir = f->root;
    }
}
